...
3. useCallback() 훅은 useMemo()와 비슷한 역할을 수행합니다. 그러나 반환하는 것은 값이 아니라 함수입니다. 의존성 배열을 매개변수로 받는 점은 useMemo와 동일합니다. 매개변수로 전달된 함수를 콜백이라고 부릅니다.

useMemo와 마찬가지로 의존성 배열 중 하나라도 변경되면 콜백 함수가 반환됩니다. 이를 통해 성능 최적화와 불필요한 재계산을 방지할 수 있습니다.

4. useRef() 훅은 레퍼런스(reference)를 사용하기 위한 훅입니다. 레퍼런스란 특정 컴포넌트에 접근할 수 있는 객체를 의미합니다. useRef() 훅은 이러한 레퍼런스 객체를 반환합니다. 반환된 레퍼런스 객체는 .current라는 속성을 가지며, 현재 참조하고 있는 엘리먼트를 가리킵니다.

예를 들어, const refContainer = useRef(initialValue);와 같이 사용합니다. 반환된 레퍼런스 객체는 컴포넌트의 전체 라이프타임 동안 유지됩니다. 즉, 컴포넌트가 마운트 해제되기 전까지 계속해서 유지됩니다. 이를 활용하여 이전 값과 현재 값의 비교 등 다양한 용도로 활용할 수 있습니다.

* 훅의 규칙
*첫 번째 규칙은 훅을 항상 최상위 레벨에서 호출해야 한다는 것입니다. 
  1.여기서 최상위 레벨은 컴포넌트의 가장 상위에 위치하는 부분을 의미합니다. 따라서, 반복문이나 조건문, 중첩된 함수 내에서 훅을 호출하는 것은 허용되지 않습니다. 
  이 규칙을 따르면 훅은 컴포넌트가 렌더링될 때마다 동일한 순서로 호출되어야 합니다.
*두 번째 규칙은 리액트 함수 컴포넌트에서만 훅을 호출해야 한다는 것입니다. 
  1.일반 자바스크립트 함수 내에서 훅을 호출하는 것은 허용되지 않습니다. 
  훅은 리액트 함수 컴포넌트나 직접 만든 커스텀 훅 내에서만 호출할 수 있습니다. 
  이 규칙을 지키면 훅이 올바르게 동작하고 예기치 않은 오류를 방지할 수 있습니다.
예시:
```
import React, { useState } from 'react';
